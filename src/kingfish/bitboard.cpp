...
static Bitboard generateSliderAttacks(Square s, Direction dir, Bitboard occ) {
    ...
    while (true) {
        ...
        if ((dir == DIR_EAST || dir == DIR_WEST) && rankDelta != 0) {
            // Rank moved
            break;
        }
    ...
    for (Square a = 0; a < 63; ++a) {
        ...
        for (Square b = a + 1; b < 64; ++b) {
            ...
            if (deltaY == 0) { // Horizontal
                ...
                // Diagonal
                if (deltaX < 0) {
                    for (Square s = a + DIR_NORTHWEST; s < b;
                         s += DIR_NORTHWEST) {
                        bb.add(s);
                    }
                } else { // deltaX > 0
                    for (Square s = a + DIR_NORTHEAST; s < b;
                         s += DIR_NORTHEAST) {
                        bb.add(s);
                    }
                }
...
static void generatePawnAttacks() {
    for (Color c = CL_WHITE; c < CL_COUNT; ++c) {
        ...
        for (Square s = 0; s < 64; ++s) {
            Bitboard sqBB = BIT(s);
            Bitboard bb =
                sqBB.shifted(leftCaptureDir) | sqBB.shifted(rightCaptureDir);
...
void generatePopCount() {
    ...
    for (int i = 0; i < 256; i++) {
        Bitboard::bitboardsBitCount[i] =
            (i & 1) + Bitboard::bitboardsBitCount[i / 2];
    }
}
