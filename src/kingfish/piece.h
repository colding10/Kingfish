...
static constexpr PieceTypeMask PTM_SLIDERS =
    bits::makeMask<PT_BISHOP, PT_ROOK, PT_QUEEN>();
static constexpr PieceTypeMask PTM_DIAGONAL =
    bits::makeMask<PT_BISHOP, PT_QUEEN>();
...
class Piece {
  ...
  public:
    inline Color getColor() const {
        return static_cast<Color>(m_Data & BITMASK(1));
    }
    inline PieceType getType() const {
        return static_cast<PieceType>(m_Data >> 1);
    }
