...
static constexpr MoveTypeMask MTM_CASTLES =
    bits::makeMask<MT_CASTLES_SHORT, MT_CASTLES_LONG>();
static constexpr MoveTypeMask MTM_CAPTURE = bits::
    makeMask<MT_SIMPLE_CAPTURE, MT_EN_PASSANT_CAPTURE, MT_PROMOTION_CAPTURE>();
static constexpr MoveTypeMask MTM_SPECIAL =
    bits::makeMask<MT_DOUBLE_PUSH,
                   MT_CASTLES_SHORT,
                   MT_CASTLES_LONG,
                   MT_EN_PASSANT_CAPTURE,
                   MT_SIMPLE_PROMOTION,
                   MT_PROMOTION_CAPTURE>();
static constexpr MoveTypeMask MTM_PROMOTION =
    bits::makeMask<MT_SIMPLE_PROMOTION, MT_PROMOTION_CAPTURE>();
static constexpr MoveTypeMask MTM_QUIET = bits::
    makeMask<MT_NORMAL, MT_CASTLES_LONG, MT_CASTLES_SHORT, MT_DOUBLE_PUSH>();
...
class Move {
  ...
  public:
    ...
    inline PieceType getPromotionPiece() const {
        return (m_Data >> 20) & BITMASK(3);
    }

    inline MoveType getType() const {
        return MoveType((m_Data >> 23) & BITMASK(6));
    }
    ...
    inline bool is(MoveTypeMask flags) const { return BIT(getType()) & flags; }
    ...
    inline Move(Square    src,
                Square    dst,
                Piece     srcPiece,
                Piece     dstPiece,
                MoveType  type,
                PieceType promPt = PT_NONE)
        : m_Data(0) {
        // ASSERT_VALID_SQUARE(src); TODO: fix later
    ...
    Move(const Position& pos,
         Square          src,
         Square          dst,
         PieceType       promotionPieceType = PT_NONE);
    ...
    inline bool operator>(const Move& rhs) const { return true; }
