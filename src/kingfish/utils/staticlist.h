#ifndef KINGFISH_STATICLIST_H
#define KINGFISH_STATICLIST_H
...
class StaticList {
  public:
    ...
    inline void insert(const TVal& val, int index) {
        KINGFISH_ASSERT(index <= m_Size && index >= 0, "Index out of bounds.");
    ...
    inline void add(const TVal& val) {
        KINGFISH_ASSERT(m_Size < capacity(), "Cannot add beyond capacity.");
    ...
    inline void addRange(TIter srcBegin, TIter srcEnd) {
        ...
        KINGFISH_ASSERT(delta + m_Size <= capacity(),
                        "Cannot add range beyond capacity.");
        std::copy(srcBegin, srcEnd, end());
        m_Size += delta;
    }

    inline void removeLast() {
        KINGFISH_ASSERT(m_Size > 0, "Trying to remove when empty.");
    ...
    inline bool contains(const TVal& val) const { return indexOf(val) != -1; }
    ...
    inline TVal& operator[](int index) {
        return *std::launder(reinterpret_cast<T*>(&m_Arr[index]));
    }
    inline const TVal& operator[](int index) const {
        return *std::launder(reinterpret_cast<const T*>(&m_Arr[index]));
    }
