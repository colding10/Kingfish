#ifndef KINGFISH_MOVEGEN_H
#define KINGFISH_MOVEGEN_H
...
void generatePawnMoves(const Position &pos, MoveList &ml) {
    ...
    constexpr bool GEN_SIMPLE_PROMOTIONS =
        ALLOWED_FLAGS & BIT(MT_SIMPLE_PROMOTION);
    ...
    if constexpr (GEN_SIMPLE_PROMOTIONS) {
        ...
        for (auto s : promotingPawns) {
            for (PieceType pt = PT_QUEEN; pt >= PT_KNIGHT; --pt) {
                ...
                Move   move(
                    s, dst, SRC_PIECE, PIECE_NONE, MT_SIMPLE_PROMOTION, pt);
                ml.add(move);
            }
        }
    }
    ...
    if constexpr (GEN_EP) {
        ...
        if (epSquare != SQ_INVALID) {
            ...
            Bitboard epCapturers = epBB.shifted<-LEFT_CAPT_DIR>() |
                                   epBB.shifted<-RIGHT_CAPT_DIR>();
            epCapturers &= pawns;

            for (auto s : epCapturers) {
                Move move(
                    s, epSquare, SRC_PIECE, PIECE_NONE, MT_EN_PASSANT_CAPTURE);
                ml.add(move);
            }
        }
    }
...
void generateKnightMoves(const Position &pos, MoveList &ml) {
    ...
    if constexpr (GEN_SIMPLE_CAPTURES) {
        ...
        for (auto ks : knights) {
            ...
            for (auto s : attacks) {
                Move move(
                    ks, s, SRC_PIECE, pos.getPieceAt(s), MT_SIMPLE_CAPTURE);
                ml.add(move);
            }
        }
    }
...
void generateCastles(const Position &pos, MoveList &ml) {
    ...
    Move move(SRC,
              ...
              SRC_PIECE,
              PIECE_NONE,
              S == SIDE_KING ? MT_CASTLES_SHORT : MT_CASTLES_LONG);
    ml.add(move);
}
...
template <Color         C,
          MoveTypeMask  ALLOWED_MOVE_TYPES,
          PieceTypeMask ALLOWED_PIECE_TYPES,
 ...
 */
template <MoveTypeMask  ALLOWED_MOVE_TYPES  = MTM_ALL,
          PieceTypeMask ALLOWED_PIECE_TYPES = PTM_ALL,
          bool          PSEUDO_LEGAL        = false>
