...
bool Game::isValidPawnMove(Board* board, Location starting, Location ending, PieceColor starting_color) {
    ...
    if (ending_piece == 0) {  // open piece
        if (opposite_color == WHITE) {
            if (
                starting.X == 6 && starting.X - ending.X == 2 && starting.Y == ending.Y && !board->getPieceAt({5, starting.Y})) {
                return true;
            }
            if (
                starting.X - ending.X == 1 && starting.Y == ending.Y) {
                return true;
            }
        } else if (opposite_color == BLACK) {
            if (
                starting.X == 1 && ending.X - starting.X == 2 && starting.Y == ending.Y && !board->getPieceAt({2, starting.Y})) {
                return true;
            }
            if (
                ending.X - starting.X == 1 && starting.Y == ending.Y) {
                return true;
            }
        }
...
bool Game::isValidBishopMove(Board* board, Location starting, Location ending, PieceColor starting_color) {
    if (starting.X + starting.Y != ending.X + ending.Y && starting.X - starting.Y != ending.X - ending.Y) {
        return false;
    }
...
bool Game::isValidKingMove(Board* board, Location starting, Location ending) {
    ...
    if (starting.X == 0 && starting.Y == 4) {
        if (ending.X == 0 && ending.Y == 6 && !board->getPieceAt({0, 5}) && Pieces::getPieceClass(board->getPieceAt({0, 7})) == ROOK) {
            return true;
        }
        if (ending.X == 0 && ending.Y == 2 && !board->getPieceAt({0, 3}) && Pieces::getPieceClass(board->getPieceAt({0, 0})) == ROOK) {
            return true;
        }
    }
    if (starting.X == 7 && starting.Y == 4) {
        if (ending.X == 7 && ending.Y == 6 && !board->getPieceAt({7, 5}) && Pieces::getPieceClass(board->getPieceAt({7, 7})) == ROOK) {
            return true;
        }
        if (ending.X == 7 && ending.Y == 2 && !board->getPieceAt({7, 3}) && Pieces::getPieceClass(board->getPieceAt({7, 0})) == ROOK) {
            return true;
        }
    }
